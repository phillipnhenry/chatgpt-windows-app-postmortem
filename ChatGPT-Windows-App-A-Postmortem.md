...
The missing visible histories are user-observed; the package/executable identity and the task's missing browser runtime were independently replayed through local process inspection and the task-control interface. A complete deferred-tool discovery inside Brand Navigation returned no name or description matching `node_repl`, CUA, Computer Use, or browser control: `[]`. Neither `mcp__node_repl__js` nor `mcp__cua_repl` was discoverable, so this is not a case where the task merely failed to request a deferred tool.

The task's recorded cwd was `C:\CodeProjects\CodeWorksLabs\Discourse`, a native Windows path rather than WSL. Its repository operations had to explicitly target the real repository at `C:\CodeProjects\Products\Discourse Brand Navigation`. A WSL-root restriction therefore does not explain the failure; the stale task cwd is a separate persisted-metadata defect. This is not a missing Node installation, npm package, or `PATH` entry. The app did not attach the host-provided Computer Use runtime to the resumed task. The update did not repair the damaged task binding. It added another failed automatic-restart event and left the user facing an apparently empty visible task history.
